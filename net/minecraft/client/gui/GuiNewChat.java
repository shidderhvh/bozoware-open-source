...
import bozoware.impl.module.visual.NameHider;
...
//                                BlurUtil.blurArea(i2, j2 - 9, l + 4, 9);
                                String s;
                                if(BozoWare.getInstance().getModuleManager().getModuleByClass.apply(NameHider.class).isModuleToggled())
                                s = chatline.getChatComponent().getFormattedText().replaceAll(mc.thePlayer.getName(), "\247c" + BozoWare.BozoUserName);
                                else
                ...
                if (flag)
                {
                    int k2 = BozoWare.getInstance().getFontManager().largeFontRenderer.getHeight() + 4;
