...
import bozoware.impl.module.visual.Crosshair;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        Render2DEvent render2DEvent = new Render2DEvent(scaledresolution);
...
//            this.drawTexturedModalRect(i - 91, sr.getScaledHeight() - 22, 0, 0, 182, 22);
            RenderUtil.drawSmoothRoundedRect(i - 91, sr.getScaledHeight() - 22, i + 91, sr.getScaledHeight() - 2, 5, 0x60000000);
            RenderUtil.drawSmoothRoundedRectWithWidth(i - 91, sr.getScaledHeight() - 2, 182, 2, 1, -1);
            RenderUtil.drawSmoothRoundedRectWithWidth(i - 91 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 2, 23, 2, 0, HUD.getInstance().bozoColor);
//            RenderUtil.drawBoxOutline(i - 91 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 22 - 1, i - 68 - 1 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 2, -1,2);
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_)
    {
        ...
        for (Object score1 : arraylist1)
        {
            ...
            BlurUtil.blurArea(j - 2, l, i1, this.getFontRenderer().FONT_HEIGHT);
            if (s1.contains("hypixel")) {
                s1 = "\247l\247eBozoWare";
            }
            mc.fontRendererObj.drawStringWithShadow(s1.replaceAll("eu.loyisa.cn", "\247bBozoWare"), j, l, s1.equals("BozoWare") ? HUD.getInstance().bozoColor : -1);
//            this.getFontRenderer().drawString(s2, i1 - this.getFontRenderer().getStringWidth(s2), l, HUD.getInstance().bozoColor2); //553648127

            if (k == arraylist1.size())
            {
                String s3 = p_180475_1_.getDisplayName().replaceAll("eu.loyisa.cn", "BozoWare");
                if (s3.contains("hypixel")) {
                    s3 = "\247l\247eBozoWare";
                }
                drawRect(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 1, i1, l - 1, 1610612736);
                drawRect(j - 2, l - 1, i1, l, 1342177280);
//                BlurUtil.blurArea(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 1, i1, l - 1);
                mc.fontRendererObj.drawStringWithShadow(s3, j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, l - this.getFontRenderer().FONT_HEIGHT, HUD.getInstance().bozoColor);
            }
        }
    }
